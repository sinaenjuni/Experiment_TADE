...
class Discriminator(nn.Module):
    ...
    def __init__(self, nc, nf, num_classes):
        ...
        self.layer3 = self.getCBL(nf*16, nf*32, 3, 1, 1, False)

        self.layer4 = self.getCBL(nf*32, nf*64, 3, 2, 1, False)

        # self.output_layer = nn.Sequential(nn.Linear(4 * 4 * nf*64, 1),
        #                                nn.Sigmoid())
        self.output_layer = nn.Sequential(nn.Linear(4 * 4 * nf*64, 1))
...
if __name__ == "__main__":
    ...
    train_loader = ImbalanceMNISTDataLoader(data_dir="../../data", batch_size=batch_size, shuffle=True, num_workers=1, training=True, balanced=False,
                 retain_epoch_size=True, imb_factor=1)
    ...
    fixed_noise = torch.randn(10, 100).repeat(10, 1).to(device)
    ...
    print(g_out.size())
    d_out = D(g_out, fixed_condition.view(-1, 10, 1, 1))
    print(d_out.size())
